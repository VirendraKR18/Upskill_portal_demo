...
PropelIQ-Windsurf is an AI-powered development framework that automates software development workflows from requirements gathering to implementation. It provides structured workflows for generating specifications, user stories, tasks, and code while enforcing comprehensive development standards across multiple languages and frameworks. The system combines business analysis, architecture design, and quality assurance into a unified platform for rapid, high-quality software delivery.

Workflows automatically evaluate AI/GenAI suitability during architecture design and adapt task planning, testing, and code review processes accordingly when AI components are identified.

## Setup

PropelIQ-Windsurf requires initial configuration to integrate with your project and enable AI-powered workflow automation. This setup process configures the framework files, API credentials, and MCP server integrations necessary for full functionality.

### Prerequisites

Before beginning setup, ensure you have:
- Windsurf installed on your system
...
- `.windsurf/` - Contains workflow definitions and development rules
...
Choose one of the following options to configure MCP (Model Context Protocol) servers:

**Option 1: Manual Configuration**

1. Navigate to `C:\Users\<YourUsername>\.codeium\windsurf\`

   > **Note:** The `.codeium\windsurf` folder is automatically created when Windsurf is installed.

2. Copy `mcp_config-example.json` to this location and rename it to `mcp_config.json`

**Option 2: UI-Based Configuration**

1. Launch Windsurf
2. Press `Ctrl + L` to open "Code with Cascasde"
3. Click the `MCPs` (plug icon) 
4. Click configuration icon (top right-most corner)
6. Copy the contents from `mcp_config-example.json` into `mcp_config.json`
7. Save the file and restart Windsurf
...
1. Open "Code with Cascade" by pressing `Ctrl + L`
2. Type `\` (backslash) to view available workflows
3. Confirm that all workflows from `.windsurf/workflows/` appear in the list

If workflows are visible, your setup is complete and PropelIQ-Windsurf is ready to use.

## Workflows

| Workflow | Description | Input | Output | Usage Example |
|----------|-------------|-------|--------|---------------|
...
*For detailed workflow documentation, refer to individual workflow files in `.windsurf/workflows/`. For comprehensive rule details, see files in `.windsurf/rules/`. For template structures, see files in `.propel/templates/`.*
